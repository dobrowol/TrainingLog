...
public interface TrainingGoalJoinDAO {
    ...
    @Query("SELECT goal_table.* FROM goal_table  INNER JOIN training_goal_join ON goal_table.goalId=training_goal_join.goal WHERE training_goal_join.trainingId=:trainingId ")
    LiveData<List<Goal>> getGoalsForTraining(final String trainingId);

    @Query("SELECT training_table.* FROM training_table  INNER JOIN training_goal_join ON training_table.id=training_goal_join.trainingId WHERE training_goal_join.goal=:goalId ")
    ...
    @Query("SELECT training_goal_join.* FROM training_goal_join WHERE training_goal_join.trainingId=:trainingId AND training_goal_join.goal=:goalId")
    ...
    @Query("SELECT training_goal_join.id, trainingId, goal, load FROM training_goal_join  ")
    LiveData<List<TrainingGoalJoin>> getAllGroupedByGoal();

    @Query("DELETE FROM training_goal_join WHERE trainingId=:trainingId AND goal=:goalId")
    void delete(String trainingId, String goalId);
}
