...
public interface TrainingExerciseJoinDAO {
        ...
        @Query("SELECT training_table.* FROM training_table  INNER JOIN training_exercise_join ON training_table.id=training_exercise_join.trainingId WHERE training_exercise_join.exerciseId=:exerciseId ORDER BY training_table.date")
                LiveData<List<Training>> getTrainingsForExercise(final String exerciseId);
